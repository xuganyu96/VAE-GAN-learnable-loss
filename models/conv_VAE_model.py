...
# VAE network. The implementation is similar to that of the demo
# provided in https://gluon.mxnet.io/chapter13_unsupervised-learning/vae-gluon.html
# but with my only twist regarding content loss and the use of convolutional
# layers

class conv_VAE(gluon.Block):
    ...
    def __init__(self, n_latent = 2,
        ...
        with self.name_scope():
            ...
            for i in range(n_hlayers):
                ...
                hlayer_n_channels = 2 ** (i + 3)
                self.encoder.add(nn.Conv2D(hlayer_n_channels, 
    ...
    def forward(self, x):
        ...
        x_hat = self.decoder(latent_z).reshape((batch_size, 
                                                self.out_n_channels, 
                                                self.out_width, 
                                                self.out_height))
        self.x_hat = x_hat
        ...
        content_loss = gloss.SigmoidBinaryCrossEntropyLoss(from_sigmoid=True)(x_hat, x)
